...
它帮助用户检查数据、配置确定性的统计模型、阅读诊断结果，并导出报告。应用支持中文和英文界面/报告文案，并提供内置合成示例数据用于演示工作流。
...
- 将 DID、IV/2SLS 和 PSM 保持为受保护的手动研究设计流程。
...
演示不需要 API key、登录系统、数据库、遥测服务或外部 LLM provider。
...
4. 使用推荐设定，或手动配置模型。
5. 检查运行前风险后运行模型。
6. 阅读结果快照、初学者指引、诊断和谨慎提示。
7. 导出简版报告、完整报告或可复现压缩包。
